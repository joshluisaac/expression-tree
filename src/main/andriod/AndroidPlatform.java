 ...
 * @brief Accounts for the user input and the ExpressionTree textual output. It organizes the data
 *     so an Android platform can easily recognize the data and create the GUI. It plays the role of
...
public class AndroidPlatform extends Platform {
  ...
  HashMap<String, CommandMenu> menu = new HashMap<String, CommandMenu>();
  ...
  /**
   * The commandMenu interface precludes the use of an excessive if-statement to activate various
   * GUI operations.
   */
  ...
  public AndroidPlatform(Object input, Object output, final Object activityParam) {
  ...
  private void enableRadioGroup(int id, boolean enable) {
    ...
    for (int i = 0; i < rg.getChildCount(); i++) (rg.getChildAt(i)).setEnabled(enable);
  }
  ...
  public String retrieveInput(boolean verbose) {
    ...
    if (verbose) words = buttonToggled() + " " + editTextInput.getText().toString();
    else words = editTextInput.getText().toString();

    words = words.trim();

    if (verbose) Log.e("AndroidPlatform", "retrieveInputVerbose " + words);
    else Log.e("AndroidPlatform", "retrieveInputSuccinct " + words);

    return words;
  }

  /** Retrieves textual input of the toggled button determined by the user. */
  ...
  /**
   * Specific for the printvisitor functionality. Allows for the addition of multiple lines to a
   * textView display.
   */
