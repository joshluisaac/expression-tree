...
public class TreeOps {
  /** Keep track of the current state of the sequence of user operations. */
  private State state;
  ...
  /**
   * Make an expression tree based on the designated @a expression using the previously designated
   * format.
   */
  ...
  /** Print the most recently created expression tree using the designated @a format. */
  public void print(String format) {
    state.print(this, format);
  }

  /**
   * Evaluate the "yield" of the most recently created expression tree using the designated @a
   * format.
   */
  ...
  public void set(String keyValuePair) throws Exception {
    ...
    if ((pos = inputString.indexOf('=')) != -1) {
      /**
       * If the position is not the first char (e.g., '=value') and position is not the last char
       * (e.g., 'key=') then split the string and set the symbol table in the interpreter
       * accordingly.
       */
  ...
  /** Set the current @a State to the designated @a newstate pointer. */
  void state(State newState) {
    this.state = newState;
  }

  /** Return the current @a ExpressionTree that's owned by this object. */
  ExpressionTree tree() {
    return tree;
  }
