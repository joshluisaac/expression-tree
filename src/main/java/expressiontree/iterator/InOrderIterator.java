 ...
 * @brief Iterates through an @a Tree using in-order traversal. Plays the role of the
 *     "ConcreteStrategy" in the Strategy pattern that defines the pre-order iteration algorithm.
 */
public class InOrderIterator implements Iterator<ExpressionTree> {
  ...
  public InOrderIterator(ExpressionTree tree) {
    if (!tree.isNull()) {
      ...
      while (!stack.peek().left().isNull()) stack.push(stack.peek().left());
    }
  }
  ...
  public ExpressionTree next() {
    ...
    if (!stack.isEmpty()) {
      ...
      if (!stack.peek().right().isNull()) {
        /**
         * Push the right child node onto the stack and pop the old parent(it's been visited now).
         */
