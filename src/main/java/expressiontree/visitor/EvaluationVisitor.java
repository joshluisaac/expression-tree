 ...
 * @brief This plays the role of a visitor for evaluating nodes in an expression tree that is being
 *     iterated in post-order fashion (and does not work correctly with any other iterator). This
 *     class plays the role of the "ConcreteVisitor" in the Visitor pattern.
 */
public class EvaluationVisitor implements Visitor {
  ...
  public void visit(CompositeNegateNode node) {
    if (stack.size() >= 1) stack.push(-stack.pop());
  }

  /** Visit a @a CompositeAddNode. */
  public void visit(CompositeAddNode node) {
    if (stack.size() >= 2) stack.push(stack.pop() + stack.pop());
  }
