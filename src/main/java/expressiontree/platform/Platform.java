...
/**
 * @class Platform
 * @brief Each platform (e.g., Android, command-line, etc.) subclasses from this Singleton, which
 *     allows a client to send and retrieve data from the expression tree processing app in a manner
 *     that isn't exposed to the bulk of the app code.
 */
public abstract class Platform {
  ...
  /** Method that sets a new Platform singleton and returns the one and only singleton instance. */
  ...
  /**
   * Implemented during the verbose android GUI. Disables unnecessary Radiobuttons so the prompt can
   * initialize the appropriate user interface.
   */
  public abstract void disableAll(boolean verbose);

  /**
   * Allows for the functionality of a print visitor within the Android Verbose mode. Continuously
   * modifies the editText instead of replacing its contents.
   */
