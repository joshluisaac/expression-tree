 ...
 * @brief Provides a concrete interface for less verbosely handling input events associated with the
 *     expression tree application. This class plays the role of "Concrete Class" in the Template
 *     Method pattern.
 */
public class SuccinctModeInputHandler extends InputHandler {
  /** This hook method succinctly prompts the user for input. */
  public void promptUser() {
    if (Platform.instance().isCommandLinePlatform()) Platform.instance().outputString("> ");
  }
  ...
  public UserCommand makeUserCommand(String userInputExpression) {
    /**
     * Create a MacroCommand by prefixing the keyword "macro" in front of the userInputExpression.
     */
