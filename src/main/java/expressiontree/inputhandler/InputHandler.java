...
/**
 * @class InputHandler
 * @brief Provides an abstract class for handling input events and commands associated with the
 *     expression tree application.
 *     <p>This class defines methods for use in the Template Method pattern that is used to process
 *     user input commands.
...
public abstract class InputHandler {
  ...
  public static InputHandler makeHandler(
    ...
    if (verbose) return new VerboseModeInputHandler();
    else return new SuccinctModeInputHandler();
  }

  /**
   * This method is called back when input is available. It implements the Template Method pattern
   * to perform the sequence of steps associated with processing expression tree application
   * commands.
   */
  public void handleInput() throws Exception {
    ...
    if (input.equals("")) InputDispatcher.instance().endInputDispatching();
    else {
  ...
  /** This hook method is a placeholder for prompting the user for input. */
  ...
  /** This hook method is a placeholder for making a command based on the user input. */
