...
/**
 * @class ComponentNode
 * @brief An abstract base class defines a simple abstract implementation of an expression tree
 *     node. This class plays the role of the "Component" in the Composite pattern. The methods in
 *     this class are not defined as pure virtual so that subclasses in the Composite pattern don't
 *     have to implement methods they don't care about.
 */
public abstract class ComponentNode {
  ...
  /**
   * Accept a visitor to perform some action on the node's item completely arbitrary visitor
   * template (throws an exception if called directly).
   */
