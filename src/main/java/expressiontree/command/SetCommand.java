...
/**
 * @class SetCommand
 * @brief Sets a variable into the SymbolTable stored inside of TreeOps. This plays the role of the
...
public class SetCommand extends UserCommand {
  ...
  /** Constructor that provides the appropriate @a TreeOps and the requested format. */
  public SetCommand(TreeOps context, String keyValuePair) {
  ...
  public void printValidCommands(boolean verboseField) {
    ...
    /**
     * If we've never had a format command called successfully, then we have a different set of
     * valid commands.
     */
    char abc[] = {'a', 'b', 'c'};
    int step = 1;
    ...
    if (!treeOps.formatted()) {
      ...
      platform.outputMenu(
          step++ + "b.", "print", "[in-order | pre-order | post-order| level-order]");
      step = 0;
    }
