 ...
 * @brief Interface for the Composite pattern that is used to contain all the operator and operand
 *     nodes in the expression tree. Plays the role of the "Abstraction" in the Bridge pattern and
 *     delegates to the appropriate "Implementor" that performs the expression tree operations,
 *     after first logging the start and finish of each call.
 */
public class InstrumentedExpressionTree extends ExpressionTree {
  /** A factory class capable of creating iterators dynamically. */
  IteratorFactory treeIteratorFactory = new IteratorFactory();

  /** Ctor that takes a @a Node * that contains all the nodes in the expression tree. */
  ...
  /** Returns the designated iterator after requesting it from factory method. */
  public Iterator<ExpressionTree> makeIterator(String traversalOrder) {
    System.out.println("starting iterator() call");
    Iterator<ExpressionTree> temp = treeIteratorFactory.makeIterator(this, traversalOrder);
